            ...
            if (($test1) -and ($Response -eq "y") -and ($Response1 -eq "y")) {
                ...
                If (Test-Connection -ComputerName $OldPCNewName -Count 5 -Delay 5 -Quiet) {
                    ...
                    if ($ReservedIP) {
                        ...
                        try {Add-DhcpServerv4Reservation -ScopeId 10.200.0.222 -IPAddress $ReservedIP -ComputerName $CurrentPC -ClientId $ClientID -Description "$CurrentPC"}
                        catch {"Not able to make a Reservation for $CurrentPC at IP $ReservedIP"}
                    }
